...
sign-in, eGovPay for every fee, and eMessage for SMS. The other six are covered by the SDK
and have gateway URLs in `.env.example`, but no workspace reads them yet. The full list, with
what each one would be used for, is in [eGov integration points](/egov/integration-points).
...
The `egov.js` SDK that every partner call goes through is documented separately at
[egov-sdk.omsimos.com](https://egov-sdk.omsimos.com).
