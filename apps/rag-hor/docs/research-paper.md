...
The platform is designed as an eGovPH-native civic service. Omsimos maintains the standalone typed [`egov.js`](https://github.com/omsimos/egov.js) SDK for nine eGovPH services; the product plans to use a deliberate subset:

- **Government data as the source of truth.** Legislative records, jurisprudence, and budget data will come from open Philippine government and civic-tech data services (§5.1), including the authenticated **eGov Compass** partner API for the national budget, rather than a model's memory or generic web content.
- **eGov AI as building blocks.** The planned refinement layer uses **AI Assistant**, **Translator**, and **Document Extractor** (§4.2, §5.3). eGov AI offers no speech-to-text, so it does not replace the transcription source, and its Filipino/Taglish quality must be validated empirically.
- **eMessage for reach.** Planned SMS notifications via eMessage turn the platform from pull to push, alerting citizens when a followed hearing is scheduled or a fact-check digest is ready.
...
The target backbone combines two open sources with the authenticated eGov Compass integration. The open sources were confirmed reachable on the verification date; Compass remains to be wired into the product.
...
**Foundation already in place (see the [eGov API reference](../../../docs/architecture.md) and [product architecture](./architecture.md)):** a Bun/Turborepo monorepo with a working transcript scraper and a typed eGov SDK covering nine services. The service contracts are available, but the eGov AI, Compass, eGovChain, eReport, and eVerify adapters are not yet wired into `rag-hor`; the phases below add those product integrations.
