...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return Response.json({ error: "Describe the business you want to start." }, { status: 400 });
  return Response.json({ conversation: createConversation(parsed.data.initialPrompt) }, { status: 201 });
}
