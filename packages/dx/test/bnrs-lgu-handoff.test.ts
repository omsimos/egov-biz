...
import { createBnrsService } from "../src/bnrs/index.js";
import { createLguService } from "../src/lgu/index.js";
...
describe("BNRS to LGU handoff", () => {
  test("uses an actor-scoped BNRS certificate with its structured business address", async () => {
    ...
    const bnrsRepository = new MemoryBnrsRepository();
    ...
    const lgu = createLguService({
      repository: new MemoryLguRepository(),
      now: () => NOW,
    });
